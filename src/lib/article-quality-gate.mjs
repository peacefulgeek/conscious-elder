 ...
 * AI writing lacks these signals; human writing has them naturally.
 */
export function voiceSignals(text) {
  const stripped = text.replace(/<[^>]+>/g, ' ');
  const lower = stripped.toLowerCase();

  // Contractions - AI avoids them unless prompted
  ...
  // Sentence length variance - AI tends toward evenly-medium sentences
...
export function runQualityGate(articleBody) {
  ...
  // Word count - strict 1,200 to 2,500
  ...
  // AI-flagged words - zero tolerance
